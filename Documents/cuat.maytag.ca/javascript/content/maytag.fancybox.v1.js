 /*!
 * The originals functions of this library have been modified for specific tasks required to complete WHPMAY-368.
 * version: 1.0 (25/06/2012)
 * Modified by: Ariel Ibarra B.
 */
 /*!
	...
	$.extend(F, {
		...
		defaults: {
			...
			tpl: {
				...
				tooltipArrow: '<div class="fancybox-arrow_tooltip"></div>'
			},
		...
		_getPosition: function (a) {
			...
			if((typeof current.top === 'undefined') || (typeof current.top === null)){
				rez = {
					position: 'absolute',
			...
			if (current.fixed && (!a || a[0] === false) && height <= viewport.h && width <= viewport.w) {
				if((current.top === 'undefined') || (current.top === null)){
					rez.left = margin[3];
					rez.top = margin[0];
				}else{
					rez.position='absoloute';
					rez.top = current.top;
					rez.left = current.left;
				}
			}			
			if((current.top === 'undefined') || (current.top === null)){
		...
		_afterZoomIn: function () {
			...
			if (current.tooltipArrow) {
				if(current.arrowDirection == "left"){
					$(current.tpl.tooltipArrow).removeClass("fancybox-arrow_tooltip").addClass("fancybox-arrow_tooltip_left").prependTo(F.wrap);
				}else{
					$(current.tpl.tooltipArrow).appendTo(F.wrap);
				}
			}
