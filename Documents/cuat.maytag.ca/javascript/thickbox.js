...
function tb_show(caption, url, imageGroup) {//function called when the user clicks on a thickbox link
	...
	console.log("vars= " + caption + ", " + url + ", " + imageGroup);
       // if (t != undefined && a != undefined && g != undefined){ 
		...
		if(tb_detectMacXFF()){
			...
			if($(this).closest('iframe').length <1){
	...
	if(params['modal'] == "false"){
		...
		if($('.modal_closeButton').html()==null){
			if($('html[lang=fr]').length>0){
				tb_closeButtonTxt="FERMER";
				tb_closeButtonWidth=60;
			}else{
				tb_closeButtonTxt="CLOSE";
		...
		if(url.indexOf('TB_iframe') != -1){
			...
			$('.sideTop').width($('#TB_frame').width()-$('.modal_closeTxt').width()-58);
		}
...
// reposistion popup if it no longer fits in the window	
	/*function resizedw(){
		tb_position();
	}
	...
	$(window).resize(function(){
	  ...
	  resizeDelay = setTimeout(resizedw, 100);
	}); */
