...
function checkCompareNow(toggleButton) {
	...
	//toggleButton.parent().find("a.compare_product_button_chkd").css("display","block");
	//Start: QC 57 compare
	...
	/*if($(compareCheckBoxVisible)[0] != undefined){
		//countCompare++;
	}*/
	...
	if(!$($(compareCheckBox)[0]).is(':checked')){		
		...
		/*var id = toggleButton.attr('id').split('_')[1];
	...
	if(itemsLength > 1 ){
		...
		//$('div.bar_compare_btn').html('<p>Compare</p>');
	}else{
...
function styleCellsAndBorders() {
	if ($("#page").hasClass("product_list_page")
			|| $("#page").hasClass("product_list_accessories_page")) {
	...
	if ($("#page").hasClass("product_list_pair_page")) {
		...
		listContainers.each( function() {
			...
			myProdSets.each( function() {
				...
				$(this).find(".tbl_cell").css("border-right", "none");
				if ((myIdx + 1) % 2 == 0) {
					$(this).find(".tbl_cell").css("border-right",
							"1px dotted #A9A9A9");
					...
					if (justBorderedRight) {
						$(this).find(".tbl_cell").css("border-left",
								"1px dotted #A9A9A9");
					}
				}
				if ((myIdx + 1) % 4 == 0) {
	...
	if ($("#page").hasClass("product_list_list_page")) {
		...
		listContainers
				.each( function() {
					var myProdSets = $(this).find("div.product_detail_set");
					myProdSets.first().find("div.row_cell").css("border-top",
							"none");
					myProdSets.last().find("div.row_cell").css("border-bottom",
							"none");
				});
	}
		...
		listContainers.each( function() {
			...
			myProdSets
					.each( function() {
						var myIdx = $(this).index();
						if ((myIdx + 1) % 2 == 0) {
							$(this).find(".row_cell").css("border-bottom",
									"1px dotted #A9A9A9");
							$(this).find(".row_cell").css("margin-bottom",
									"3px");
							$(this).next().find(".row_cell").css("border-top",
									"1px dotted #A9A9A9");
							$(this).next().find(".row_cell").css("padding-top",
									"10px");
						}
					});
